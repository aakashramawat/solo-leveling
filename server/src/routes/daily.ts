...
dailyRouter.get('/', async (_req, res) => {
  ...
  // Return task records grouped by category for undo support
...
// Body: { category: TaskCategory, done: boolean }
dailyRouter.post('/mark', async (req, res) => {
  const player = await getOrCreatePlayer();
  ...
  // Create task as completed or failed immediately
...
// Body: { taskId: string }
dailyRouter.post('/unmark', async (req, res) => {
...
// Body: { category: TaskCategory }
dailyRouter.post('/skip', async (req, res) => {
  const player = await getOrCreatePlayer();
  ...
  // Create a completed task with 0 XP (guilt-free skip)
